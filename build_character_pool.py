"""One-time script to build the static character pool from Jikan (MyAnimeList).

This is NOT part of the running app -- it's run manually, offline, whenever
you want to (re)generate the data:

    cd /Users/aaronzhang/Projects/Multiplayer-Imposter-Game
    source .venv/bin/activate
    python3 build_character_pool.py

It writes character_pool_data.py, which the running app imports as a plain
Python literal at startup -- no network calls happen during gameplay.
Takes several minutes; requests are paced to respect Jikan's rate limit.
Safe to re-run any time (e.g. after a Jikan outage) to pick up more/better
data -- it always builds a fresh file from scratch.
"""
...
OUTPUT_FILE = "character_pool_data.py"
MAX_ATTEMPTS = 5
...
async def main() -> None:
    ...
    with open(OUTPUT_FILE, "w") as f:
        f.write('"""Auto-generated by build_character_pool.py -- do not edit by hand.\n\n')
        f.write('Regenerate with: python3 build_character_pool.py\n"""\n\n')
        f.write("ANIME_POOL = ")
    ...
    if not results:
        print("\nWARNING: zero anime succeeded -- character_pool_data.py has an empty ANIME_POOL.")
        print("The game will have no characters to draw from until this is re-run successfully.")
        sys.exit(1)
