...
# Avatars supplied by hand rather than fetched: the source file already sits in
# static/avatars/, cropped, and there is nothing to look up.
#
# This list exists so re-running the script cannot delete them. Everything
# below is regenerated from AniList, so a hand-edit to avatar_options.py would
# be wiped by the next run -- exactly the trap build_character_pool.py fell
# into. Adding one here instead makes it survive.
#
# `file` must already exist in static/avatars/ as a square image.
LOCAL_ROSTER = [
    {
        "id": "potential",
        "name": "Potential Man",
...
async def main() -> None:
    ...
    # Hand-supplied avatars are appended after the fetched ones, so they keep
    # a stable position at the end of the picker rather than shuffling when a
    # download fails. A missing file is reported rather than silently emitted
    # as a broken path -- the app renders the emoji fallback for image: None.
    for entry in LOCAL_ROSTER:
        path = os.path.join(AVATAR_DIR, entry["file"])
        ...
        results.append(
            {
                ...
                "series": entry["series"],
                "emoji": entry["emoji"],
                "image": f"/static/avatars/{entry['file']}" if exists else None,
            }
        )
        if exists:
            print(f"OK   {entry['name']:14s} (local file)")
        else:
            failures.append((entry["name"], f"missing {path}"))
            print(f"FAIL {entry['name']:14s}: missing {path}")
